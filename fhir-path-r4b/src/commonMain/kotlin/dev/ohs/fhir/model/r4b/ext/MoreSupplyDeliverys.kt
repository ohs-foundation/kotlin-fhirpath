...
internal fun SupplyDelivery.getProperty(name: String): Any? = when(name) {
  "id" -> this.id
  "meta" -> this.meta
...
internal fun SupplyDelivery.hasProperty(name: String): Boolean = when(name) {
  "id" -> true
  "meta" -> true
  "implicitRules" -> true
  "language" -> true
...
internal fun SupplyDelivery.getAllChildren(): List<Any> = buildList {
  ...
  this@getAllChildren.suppliedItem?.let { add(it) }
...
internal fun SupplyDelivery.SuppliedItem.getProperty(name: String): Any? = when(name) {
  "id" -> this.id
  "extension" -> this.extension
  "modifierExtension" -> this.modifierExtension
...
internal fun SupplyDelivery.SuppliedItem.hasProperty(name: String): Boolean = when(name) {
  "id" -> true
  "extension" -> true
  "modifierExtension" -> true
  "quantity" -> true

...
internal fun ServiceRequest.getProperty(name: String): Any? = when(name) {
  "id" -> this.id
  "meta" -> this.meta
...
internal fun ServiceRequest.hasProperty(name: String): Boolean = when(name) {
  "id" -> true
  "meta" -> true
  "implicitRules" -> true
  "language" -> true
...
internal fun ServiceRequest.getAllChildren(): List<Any> = buildList {
  ...
  addAll(this@getAllChildren.locationCode)
  addAll(this@getAllChildren.locationReference)
